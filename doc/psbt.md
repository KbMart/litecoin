# PSBT Howto for Panda3 Core

Since Panda3 Core 0.17, an RPC interface exists for Partially Signed Panda3
...
PSBT is an interchange format for Panda3 transactions that are not fully signed
...
#### Multisig with multiple Panda3 Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Panda3 Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `panda3-cli` is used.
