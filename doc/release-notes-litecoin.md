Panda3 Core version 0.18.1 is now available from:

  <https://download.panda3.org/panda3-0.18.1/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/panda3-dev>
...
`/Applications/Panda3-Qt` (on Mac) or `panda3d`/`panda3-qt` (on
Linux).
...
Panda3 Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Panda3 Core on unsupported systems.

Panda3 Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Panda3 Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/panda3-project/panda3/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@panda3.org) the Panda3 Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 PD3),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in PD3 per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/panda3-project/panda3/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/panda3-project/panda3/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/panda3-project/panda3/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/panda3-project/panda3/blob/master/doc/panda3-conf.md)
  about the `panda3.conf` file describes how to use it to configure
  Panda3 Core.

- A new document introduces Panda3 Core's BIP174 [Partially-Signed
  Panda3 Transactions
  (PSBT)](https://github.com/panda3-project/panda3/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/panda3-project/panda3/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Panda3-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Panda3 Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/panda3-project/panda3/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/panda3-project/panda3/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Panda3 Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Panda3 Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/panda3-project/panda3/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/panda3-project/panda3/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart panda3d with the `-deprecatedrpc=generate`
...
- A new `panda3-wallet` tool is now distributed alongside Panda3
...
This section describes planned changes to Panda3 Core that may affect
other Panda3 software and services.

- Since version 0.16.0, Panda3 Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Panda3 Core 0.20 (expected about
  a year after 0.18), Panda3 Core will default to native segwit
  ...
  to bech32 addresses, and if the Panda3 Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Panda3 Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Panda3 Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Panda3 Core
...
- A sub-project of Panda3 Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Panda3 Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Panda3 Core's own implementation, although entropy
  gathered by Panda3 Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Panda3 Core a
...
- On macOS, Panda3 Core now opts out of application CPU throttling
