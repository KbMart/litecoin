Panda3 Core version *0.15.0* is now available from:

  <https://download.panda3.org/panda3-0.15.0/>
...
installer (on Windows) or just copy over `/Applications/Panda3-Qt` (on Mac)
or `panda3d`/`panda3-qt` (on Linux).
...
Panda3 Core is extensively tested on multiple operating systems using
...
- The `estimatesmartfee` RPC interface has been changed (See [PR 10707](https://github.com/bitcoin/bitcoin/pull/10707)):
    ...
    - If Panda3 Core has not been running for long enough and has not seen enough blocks or transactions to produce an accurate fee estimation, an error will be returned (previously a value of -1 was used to indicate an error, which could be confused for a feerate).
...
Panda3 Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitcoin/bitcoin/pull/8694), [PR 10849](https://github.com/bitcoin/bitcoin/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.

Multi-wallet is enabled by using more than one `-wallet` argument when starting Panda3, either on the command line or in the Panda3 config file.
...
Panda3 Core 0.15.0 contains the following changes to the RPC interface and `panda3-cli` for multi-wallet:

* When running Panda3 Core with a single wallet, there are **no** changes to the RPC interface or `panda3-cli`. All RPC calls and `panda3-cli` commands continue to work as before.
* When running Panda3 Core with multi-wallet, all *node-level* RPC methods continue to work as before. HTTP RPC requests should be send to the normal `<RPC IP address>:<RPC port>/` endpoint, and `panda3-cli` commands should be run as before. A *node-level* RPC method is any method which does not require access to the wallet.
* When running Panda3 Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>/` endpoint, for example `127.0.0.1:9332/wallet/wallet1.dat/`. `panda3-cli` commands should be run with a `-rpcwallet` option, for example `panda3-cli -rpcwallet=wallet1.dat getbalance`.
...
- When using Panda3 Core in multi-wallet mode, RPC requests for wallet methods must specify
